...
def run(prompt: str, *, allowed_tools: list[str]) -> dict:
    ...
    # Create a clean environment without ANTHROPIC_API_KEY to allow claude
    # to use session authentication. If a valid API key is needed, users can
    # set it explicitly in their environment.
    env = os.environ.copy()
    env.pop("ANTHROPIC_API_KEY", None)

    try:
        proc = subprocess.run(
            cmd, input=prompt, capture_output=True, text=True, check=True, env=env
        )
